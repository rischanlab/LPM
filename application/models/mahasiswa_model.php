...
class Mahasiswa_model extends CI_Model
{
	...
	function get_by_nim($nim){
		...
		$query=$this->db->query("SELECT A.NIM, A.ANGKATAN, A.NAMA,A.TELP_MHS,A.GOL_DARAH, A.BERAT, A.TINGGI,A.PEKERJAAN, A.PATH_FOTO,  DECODE(A.STATUS_KAWIN,'K','Menikah','B','Belum Menikah') STATUS_KAWIN,G.PRESTASI, G.TRANSPORTASI, G.NO, G.ALAMAT_JOGJA,G.RT_JOGJA,G.DESA_JOGJA,G.NM_KEC_JOGJA, G.NM_KAB_JOGJA, G.PATH_SK_DOKTER,G.PATH_SK_GOLONGAN_DARAH,G.PATH_SK_CUTI,G.PATH_SK_TIDAK_HAMIL, A.ALAMAT_MHS ,A.RT,A.DESA, A.HP_MHS,B.NM_PRODI, C.NM_JURUSAN, D.NM_FAK,  DECODE(A.J_KELAMIN,'L','Laki-laki','P','Perempuan') J_KELAMIN, E.NM_KAB,F.NM_PROP FROM D_MAHASISWA A, MASTER_PRODI B, MASTER_JURUSAN C, MASTER_FAK D, MD_KAB E, MD_PROP F, KKN_MHS G WHERE A.KD_KAB=E.KD_KAB AND A.KD_PRODI = B.KD_PRODI AND B.KD_JURUSAN = C.KD_JURUSAN AND C.KD_FAK = D.KD_FAK AND A.KD_PROP=F.KD_PROP AND A.NIM=G.NIM AND A.NIM='$nim'");
		return $query;
	}
